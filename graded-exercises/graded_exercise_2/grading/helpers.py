...
def get_data_PICKLED(exercise_id):
    """ Loads data required by the exercise passed as `exercise_id`. Exercise must be
    registered in `grading_params` dictionary.
    ...
    # Load data.
    data_path = exercise_id + '.pck'
    with open(os.path.join(path_grade_data, data_path), 'rb') as data_file:
        grade_data = pickle.load(data_file)
